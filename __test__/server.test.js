...
describe('API Server', () => {
  describe('Categaory routes', () => {
    ...
    it('/categories should respond with 200',() => {
      return mockRequest.get('/categories').then((results) => {
        expect(results.status).toBe(200);
      });
    });
    it('GET /categories/:id should respond with 200',() => {
      return mockRequest.post('/categories').send(obj).then((data) => {
        return mockRequest.get(`/categories/${data.body._id}`).then((results) => {
          expect(results.status).toBe(200);
    ...
    it('POST /categories should respond with 201',() => {
      return mockRequest.post('/categories').send(obj).then((results) => {
    ...
    it('PUT /categories/:id should respond with 202',() => {
      return mockRequest.post('/categories').send(obj).then((data) => {
        return mockRequest.put(`/categories/${data.body._id}`).send(newObj).then((results) => {
    ...
    it('PATCH /categories/:id should respond with 202',() => {
      return mockRequest.post('/categories').send(obj).then((data) => {
        return mockRequest.patch(`/categories/${data.body._id}`).send(newObj).then((results) => {
          expect(results.status).toBe(202);
          Object.keys(obj).forEach((key) => {
            expect(newObj[key]).toEqual(results.body[key]);
          });
    ...
    it('DELETE /categories/:id should respond with 202',() => {
      return mockRequest.post('/categories').send(obj).then((data) => {
        return mockRequest.delete(`/categories/${data.body._id}`).then((results) => {
          expect(results.status).toBe(202);
  ...
  describe('Product routes', () => {
    it('/products should respond with 200',() => {
      return mockRequest.get('/products').then((results) => {
        expect(results.status).toBe(200);
      });
    });
    it('GET /products/:id should respond with 200',() => {
      return mockRequest.post('/products').send(obj).then((data) => {
    ...
    it('POST /products should respond with 201',() => {
      return mockRequest.post('/products').send(obj).then((results) => {
        expect(results.status).toBe(201);
        Object.keys(obj).forEach((key) => {
          expect(obj[key]).toEqual(results.body[key]);
        });
    ...
    it('PUT /products/:id should respond with 202',() => {
      return mockRequest.post('/products').send(obj).then((data) => {
        return mockRequest.put(`/products/${data.body._id}`).send(newObj).then((results) => {
          expect(results.status).toBe(202);
          Object.keys(obj).forEach((key) => {
            expect(newObj[key]).toEqual(results.body[key]);
          });
    ...
    it('PATCH /products/:id should respond with 202',() => {
      return mockRequest.post('/products').send(obj).then((data) => {
        return mockRequest.patch(`/products/${data.body._id}`).send(newObj).then((results) => {
          expect(results.status).toBe(202);
          Object.keys(obj).forEach((key) => {
            expect(newObj[key]).toEqual(results.body[key]);
          });
    ...
    it('DELETE /products/:id should respond with 202',() => {
      return mockRequest.post('/products').send(obj).then((data) => {
        return mockRequest.delete(`/products/${data.body._id}`).then((results) => {
          expect(results.status).toBe(202);
          expect({}).toEqual(results.body);
        });
      });
